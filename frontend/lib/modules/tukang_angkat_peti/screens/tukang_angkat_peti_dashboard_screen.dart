...
class TukangAngkatPetiDashboardScreen extends StatefulWidget {
  const TukangAngkatPetiDashboardScreen({super.key});

  @override
  State<TukangAngkatPetiDashboardScreen> createState() => _TukangAngkatPetiDashboardScreenState();
}

class _TukangAngkatPetiDashboardScreenState extends State<TukangAngkatPetiDashboardScreen> {
  final ApiClient _api = ApiClient();
  bool _isLoading = true;
  static const _roleColor = AppColors.roleTukangAngkatPeti;
  ...
  Widget build(BuildContext context) {
    final active = _assignments.where((a) => a['status'] == 'confirmed' || a['status'] == 'pending').length;
    final done = _assignments.where((a) => a['status'] == 'completed').length;
        ...
        child: _isLoading
            ...
            : ListView(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      _statCard('Tugas\nAktif', active, Icons.assignment, _roleColor),
                      const SizedBox(width: 12),
                      _statCard('Selesai', done, Icons.check_circle, Colors.green),
                    ],
                  ),
